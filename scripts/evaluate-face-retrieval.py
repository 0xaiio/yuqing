...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate face retrieval quality on the local gallery.")
    parser.add_argument("--top-k", nargs="+", type=int, default=[1, 5, 10], help="Top-K hit rates to report.")
    parser.add_argument(
        "--retrieval-limit",
        type=int,
        default=200,
        help="Maximum number of retrieved photos per query.",
    )
    parser.add_argument(
        "--person-limit",
        type=int,
...
def collect_ground_truth(repository: GalleryRepository, person_id: int) -> set[int]:
    ...
    relevant_photo_ids: set[int] = set()
    for photo in repository.list_searchable_photos(limit=5000):
        if cluster_labels.intersection(decode_json_list(photo.face_clusters)):
            if photo.id:
                relevant_photo_ids.add(photo.id)
    return relevant_photo_ids
...
def main() -> int:
    ...
    with Session(engine) as session:
        ...
        for person in people:
            relevant_photo_ids = collect_ground_truth(repository, person.id or 0)
            ...
            for sample in repository.list_person_samples(person.id or 0):
                ...
                ranks = [
                    ...
                    if hit.photo.id in relevant_photo_ids
                ]
                ...
                report["queries"].append(
                    {
                        ...
                        "sample_id": sample.id,
                        "best_rank": best_rank,
                        "relevant_photo_count": len(relevant_photo_ids),
        ...
        if not total_queries:
            print("No evaluable person queries were found. Add person samples and linked photos first.")
            return 0
    ...
    print("Face Retrieval Evaluation")
